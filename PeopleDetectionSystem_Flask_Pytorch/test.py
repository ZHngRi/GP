...
def ffmpeg_cmd(id, ip):
    ...
    command = (
        f"ffmpeg -f rawvideo -r 30 -pixel_format bgr24 -video_size {int(capture.get(3))}x{int(capture.get(4))} -framerate 30 "
...
def init(id, ip):
    ...
    # 初始化2个撞线polygon
    list_pts_blue = [[int(capture.get(4)) / 2 - 10, 0], [int(capture.get(4)) / 2 - 10, int(capture.get(3))],
                     [int(capture.get(4)) / 2, 0], [int(capture.get(4)) / 2, int(capture.get(3))]]
    ...
    list_pts_yellow = [[int(capture.get(4)) / 2 + 10, 0], [int(capture.get(4)) / 2 + 10, int(capture.get(3))],
                       [int(capture.get(4)) / 2, 0], [int(capture.get(4)) / 2, int(capture.get(3))]]
            ...
            for item_bbox in list_bboxs:
                ...
                if polygon_mask_blue_and_yellow[y, x] == 1:
                    ...
                    if track_id in list_overlapping_blue_polygon:
                        ...
                        req = requests.post("http://localhost:3001/api/detection/update",
                                            json=data)  # 发post请求,以json字符串参数格式
                        print(req)
...
if __name__ == '__main__':
    ...
    mask_image_temp = np.zeros((int(capture.get(3)), int(capture.get(4))), dtype=np.uint8)
    print('mask_image_temp: ',mask_image_temp)
    ...
    polygon_blue_value_1 = polygon_blue_value_1[:, :, np.newaxis]
    print('polygon_blue_value_1: ', polygon_blue_value_1)
    print('polygon_blue_value_1.shape: ', polygon_blue_value_1.shape)
    '''的维度增加了一个新的轴，目的是为了适应后续的操作。'''
    ...
    polygon_yellow_value_2 = polygon_yellow_value_2[:, :, np.newaxis]
    '''的维度增加了一个新的轴，目的是为了适应后续的操作。'''
    print('polygon_yellow_value_2: ', polygon_yellow_value_2)
    print('polygon_yellow_value_2.shape: ', polygon_yellow_value_2.shape)
    # 撞线检测用mask，包含2个polygon，（值范围 0、1、2），供撞线计算使用
    polygon_mask_blue_and_yellow = polygon_blue_value_1 + polygon_yellow_value_2
    print('polygon_mask_blue_and_yellow: ', polygon_mask_blue_and_yellow)
    print('polygon_mask_blue_and_yellow.shape: ', polygon_mask_blue_and_yellow.shape)
